...
import {
  ...
  googleComputeRoutes,
} from "../_shared/geo/providers/googleRoutes.ts";
import {
  googleGeocode,
  googleReverseGeocode,
} from "../_shared/geo/providers/googleGeocoding.ts";
import {
  mapboxDirections,
  mapboxGeocode,
  mapboxMatrix,
  mapboxReverse,
} from "../_shared/geo/providers/mapbox.ts";
import {
  hereGeocode,
  hereRevGeocode,
  hereRoutes,
} from "../_shared/geo/providers/here.ts";
import {
...
import { buildNoProviderErrorResponse } from "./providerPolicy.ts";
...
function endpointHint(provider: ProviderCode, action: Action): string {
  switch (provider) {
    case "google":
      if (action === "route" || action === "matrix") {
        return "routes.googleapis.com/directions/v2";
      }
...
export default Deno.serve((req: Request) =>
  withRequestContext("geo", req, async (ctx) => {
    if (req.method !== "POST") {
    ...
    if (!auth.user) {
      return errorJson(auth.error ?? "Unauthorized", 401, "unauthorized");
    }
    ...
    if (
      action !== "route" && action !== "geocode" && action !== "reverse" &&
      action !== "matrix"
    ) {
    ...
    const initialExclude: ProviderCode[] = Array.isArray(body?.exclude)
      ? (body.exclude.map(normalizeProviderCode).filter(
        Boolean,
      ) as ProviderCode[])
      : [];
    const selectionState = buildGeoProviderSelectionState({
      renderer,
      initialExclude,
      hasGeoServerKey: providerHasServerKey,
    });
    const exclude = [...selectionState.exclude];
    ...
    async function logAttempt(params: {
      ...
      try {
        await service.from("maps_requests_log").insert({
          ...
          tried_providers: (params.triedProviders ?? []).length
            ? (params.triedProviders ?? [])
            : null,
          request_summary: {
            endpoint: endpointHint(params.provider, action),
          ...
          attempt_number: Math.max(
            1,
            Math.min(50, Math.trunc(params.attemptNumber)),
          ),
    ...
    async function cacheGet(cacheKey: string) {
      try {
        const { data, error } = await service.rpc("geo_cache_get_v1", {
    ...
    async function cachePut(
      ...
      try {
        ...
        await service.rpc("geo_cache_put_v1", {
          ...
          p_ttl_seconds: Math.max(
            60,
            Math.min(60 * 60 * 24 * 7, Math.trunc(ttlSeconds)),
          ),
        });
      } catch {
        // ignore
    ...
    function allFailuresNoRoute(): boolean {
      return failures.length > 0 &&
        failures.every((f) => f.kind === "no_route");
    }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (!provider) {
        ...
        console.warn(
          ...
          JSON.stringify({
            ...
            renderer,
            exclude: pickExclude,
            compliance_excluded: selectionState.complianceExcluded,
            missing_server_keys: selectionState.missingServerKeys,
          }),
        );
        return buildNoProviderErrorResponse({
          action,
          capability,
          renderer,
          ...
          selectionState: {
            exclude: pickExclude,
            complianceExcluded: selectionState.complianceExcluded,
            missingServerKeys: selectionState.missingServerKeys,
          },
        });
      }
      tried.push(provider);
      ...
      const language = clampLanguage(
        body?.language ?? defaults?.language ?? "ar",
      );
      ...
      const cacheEnabled = Boolean(defaults?.cache_enabled) &&
        isFiniteNumber(defaults?.cache_ttl_seconds) &&
        defaults.cache_ttl_seconds > 0;
      const cacheTtlSeconds = cacheEnabled
        ? Math.min(
          Math.max(60, Math.trunc(defaults!.cache_ttl_seconds!)),
          60 * 60 * 24 * 7,
        )
        : 0;
      ...
      try {
        if (action === "route") {
          ...
          if (!origin || !destination) {
            return errorJson("Invalid origin/destination", 400, "bad_coords");
          }

          const profile =
            (body?.profile === "walking" || body?.profile === "cycling")
              ? body.profile
              : "driving";
              ...
              await logAttempt({
                ...
                responseSummary: {
                  distance_meters: Number(
                    (cached as any)?.distance_meters ??
                      (cached as any)?.distanceMeters ?? null,
                  ),
                },
              });
              return json({
          ...
          let raw: unknown;
          if (provider === "google") {
            const travelMode = profile === "walking"
              ? "WALK"
              : profile === "cycling"
              ? "BICYCLE"
              : "DRIVE";
            ...
            const mbProfile = profile === "walking"
              ? "walking"
              : profile === "cycling"
              ? "cycling"
              : "driving";
            ...
            const transportMode = profile === "walking"
              ? "pedestrian"
              : profile === "cycling"
              ? "bicycle"
              : "car";
          ...
          await logAttempt({
            ...
            responseSummary: {
              distance_meters: Number(
                (normalized as any)?.distance_meters ?? null,
              ),
              duration_seconds: Number(
                (normalized as any)?.duration_seconds ?? null,
              ),
            },
          });
          return json({
        ...
        if (action === "geocode") {
          ...
          if (!query || query.length > 256) {
            return errorJson("Invalid query", 400, "bad_query");
          }
          ...
          if (provider === "google") {
            ...
            const out = await hereGeocode({
              apiKey: key,
              query,
              language,
              limit,
              inFilter: `countryCode:${region === "IQ" ? "IRQ" : region}`,
            });
            raw = out.raw;
            normalized = out.normalized;
          } else {
          ...
          await logAttempt({
            ...
            responseSummary: {
              results: Array.isArray(normalized)
                ? normalized.length
                : undefined,
            },
          });
        ...
        if (action === "reverse") {
          ...
          if (provider === "google") {
            ...
            const out = await hereRevGeocode({
              apiKey: key,
              at,
              language,
              limit,
        ...
        if (action === "matrix") {
          const origins = Array.isArray(body?.origins)
            ? body.origins.map(validateLatLng).filter(Boolean) as LatLng[]
            : [];
          const destinations = Array.isArray(body?.destinations)
            ? body.destinations.map(validateLatLng).filter(Boolean) as LatLng[]
            : [];
          if (!origins.length || !destinations.length) {
            return errorJson("Invalid origins/destinations", 400, "bad_coords");
          }
          if (origins.length > 25 || destinations.length > 25) {
            return errorJson(
              "Matrix too large (max 25x25)",
              400,
              "matrix_too_large",
            );
          }
          ...
          if (provider === "google") {
            ...
            const destinationsIdx = destinations.map((_, i) =>
              i + origins.length
            );
        ...
        const retryAfterSeconds =
          typeof errAny?.rateLimit?.retryAfterSeconds === "number" &&
            Number.isFinite(errAny.rateLimit.retryAfterSeconds)
        ...
        const httpStatus = isTimeout
          ...
          : (typeof errAny?.httpStatus === "number" &&
              Number.isFinite(errAny.httpStatus)
            ? errAny.httpStatus
            : (parsedStatus ?? 502));
        const isInternal = message.startsWith("provider_unsupported_");
        const noRoute = action === "route" &&
          (
        ...
        if (httpStatus === 429) {
          ...
          baseCooldownSeconds = retryAfterSeconds != null
            ? Math.max(60, Math.min(86400, retryAfterSeconds))
            : 600;
        ...
        const errorCode = noRoute
          ? "no_route_found"
          : (parsedStatus
            ? `upstream_http_${parsedStatus}`
            : (isTimeout ? "timeout" : "upstream_error"));

        const shouldRecordHealth = !isInternal &&
          (isTimeout || httpStatus === 429 || httpStatus === 503 ||
            httpStatus === 401 || httpStatus === 403 || httpStatus >= 500);
        ...
        await logAttempt({
          ...
          responseSummary: Object.keys(responseSummary).length
            ? responseSummary
            : undefined,
        });
        // Try next provider.
